...
namespace PathwinderTest
{
    ...
    TEST_CASE(FilesystemRule_DirectoryCompare_EqualCaseInsensitive)
    {
        ...
        TEST_ASSERT(FilesystemRule::EDirectoryCompareResult::Equal == filesystemRule.DirectoryCompareWithOrigin(kOriginCompareDirectory));
        TEST_ASSERT(FilesystemRule::EDirectoryCompareResult::Equal == filesystemRule.DirectoryCompareWithTarget(kTargetCompareDirectory));
    }
    ...
    // Verifies that rules correctly identify paths that are in scope for redirection and recognizes those paths that are not in scope.
    // In this case the filesystem rule being tested does not use any file patterns.
    TEST_CASE(FilesystemRule_IsPathInScope_WithoutFilePatterns)
    {
        constexpr std::wstring_view kOriginDirectory = L"C:\\Directory\\Origin";
        constexpr std::wstring_view kTargetDirectory = L"D:\\AnotherDirectory\\Target";

        constexpr std::wstring_view kInScopePaths[] = {
            L"C:\\Directory\\Origin\\file.txt",
            L"C:\\Directory\\Origin\\Subdir1\\Subdir2\\file",
        };

        constexpr std::wstring_view kNotInScopePaths[] = {
            L"",
            L"C:\\Directory",
            L"C:\\Directory\\Origin",
            L"C:\\Directory\\Origin2",
            L"C:\\UnrelatedDirectory\\Origin",
        };

        const FilesystemRule filesystemRule(kOriginDirectory, kTargetDirectory);

        for (const auto& inScopePath : kInScopePaths)
            TEST_ASSERT(true == filesystemRule.IsPathInScope(inScopePath));

        for (const auto& notInScopePath : kNotInScopePaths)
            TEST_ASSERT(false == filesystemRule.IsPathInScope(notInScopePath));
    }

    // Verifies that rules correctly identify paths that are in scope for redirection and recognizes those paths that are not in scope.
    // In this case the filesystem rule being tested uses file patterns.
    TEST_CASE(FilesystemRule_IsPathInScope_WithFilePatterns)
    {
        constexpr std::wstring_view kOriginDirectory = L"C:\\Directory\\Origin";
        constexpr std::wstring_view kTargetDirectory = L"D:\\AnotherDirectory\\Target";
        std::vector<std::wstring> filePatterns = {L"A*"};

        constexpr std::wstring_view kInScopePaths[] = {
            L"C:\\Directory\\Origin\\A",
            L"C:\\Directory\\Origin\\Afile.txt",
            L"C:\\Directory\\Origin\\Asubdir\\Bsubdir\\file.txt"
        };
