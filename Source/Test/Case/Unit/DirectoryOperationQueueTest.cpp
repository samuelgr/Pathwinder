 ...
 *   Unit tests for queue-like data structures that contribute to directory enumeration
 *   implementation.
 **************************************************************************************************/
...
namespace PathwinderTest
{
  ...
  /// Generates and returns a filesystem rule that is intended to function as a file pattern
  /// source. For these tests the origin and target directories are not useful.
  /// @param [in] filePattern File pattern to include with the rule.
  /// @return Filesystem rule object with the specified file pattern.
  static inline FilesystemRule CreateFilePatternSourceRule(
      std::wstring_view filePattern, ERedirectMode redirectMode = ERedirectMode::Simple)
  {
    return FilesystemRule(filePattern, L"", L"", {std::wstring(filePattern)}, redirectMode);
  }

  /// Generates and returns a single directory enumeration instruction that basically acts as a
  /// no-op and includes all filenames. For these tests the directory path source is not useful
  /// because it is consumed before an operation queue is created to provide it with an absolute
  /// path.
  ...
  /// filenames that match a file pattern associated with the specified rule. For these tests the
  /// directory path source is not useful because it is consumed before an operation queue is
  /// created to provide it with an absolute path.
  ...
  /// Generates and returns a single directory enumeration instruction that includes only those
  /// filenames that match a file pattern associated with the applicable rules in the specified
  /// container. For these tests the directory path source is not useful because it is consumed
  /// before an operation queue is created to provide it with an absolute path.
  ...
  /// Generates and returns a single directory enumeration instruction that includes only those
  /// filenames that do not match a file pattern associated with the specified rule. For these
  /// tests the directory path source is not useful because it is consumed before an operation queue
  /// is created to provide it with an absolute path.
  /// @param [in] filePatternSource Filesystem rule used to determine whether or not a
  /// filename should be included in the enumeration result.
  ...
  /// Generates and returns a single directory enumeration instruction that includes only those
  /// filenames that do not match a file pattern associated with the applicable rules in the
  /// specified container. For these tests the directory path source is not useful because it is
  /// consumed before an operation queue is created to provide it with an absolute path.
  /// @param [in] filePatternSource Filesystem rule container used to determine whether or not a
  /// filename should be included in the enumeration result.
  /// @param [in] queryRuleSelectionMode Filtering mode for determining which rules in the
  /// container should be checked. By default, all rules are checked.
  ...
  // Creates a directory with a small number of files, gets part-way through enumerating them all,
  // and then restarts the scan. After the restart all the files should be enumerated.
  ...
  // Creates a directory with a small number of files and expects that only files that match the
  // file pattern, supplied as part of the original query, are enumerated.
  TEST_CASE(EnumerationQueue_EnumerateOnlyQueryMatchingFiles)
  {
    ...
    constexpr std::wstring_view kNonMatchingFileNames[] = {
        L"SomeOtherFile.bin", L"File0.log", L"Program.exe"};
  ...
  // Creates a directory with a small number of files, enumerates all of them, and then restarts
  // the scan but with a different file pattern. Both file patterns should be honored.
  TEST_CASE(EnumerationQueue_EnumerateWithDifferentFilePatternOnRestart)
  {
    ...
    constexpr std::wstring_view kSecondFilePatternMatches[] = {L"asdfZ.txt", L"zZz.txt"};
  ...
  // Creates a directory with a small number of files and expects that only files that match the
  // file pattern, supplied in a filesystem rule, are enumerated.
  TEST_CASE(EnumerationQueue_EnumerateOnlySingleRuleMatchingFiles)
  {
  ...
  // Creates a directory with a small number of files and expects that only files that match the
  // file patterns, supplied in multiple filesystem rules, are enumerated.
  TEST_CASE(EnumerationQueue_EnumerateOnlyMultiRuleMatchingFiles)
  {
    constexpr std::wstring_view kDirectoryName = L"C:\\Directory";
    constexpr std::wstring_view kMatchingFileNames[] = {
        L"File0.log",
    ...
    for (auto fileName : kMatchingFileNames)
      filePatternSource.EmplaceRule(CreateFilePatternSourceRule(fileName));
  ...
  // Creates a directory with a small number of files and expects that only files that match the
  // file patterns, supplied both via a filesystem rule and via query, are enumerated.
  ...
  // Creates a directory with a small number of files and expects that none are enumerated due to
  // no matches with the file pattern supplied within a filesystem rule. In this case the
  // instruction specifies to include all matching files, so the file pattern is one that does not
  // match.
  ...
  // Creates a directory with a small number of files and expects that none are enumerated due to
  // no matches with the file pattern supplied within a filesystem rule. In this case the
  // instruction specifies to exclude all matching files, so the file pattern is one that does
  // match.
  ...
  // Attempts to enumerate a directory that does not exist. This should result in the enumeration
  ...
  // Enumerates the parent directory of a single filesystem rule's origin directory such that the
  // rule's origin directory and target directory both exist in the filesystem. That origin
  // directory should be the only item enumerated.
  ...
  // Enumerates the parent directory of a single filesystem rule's origin directory such that the
  // rule's origin directory and target directory both exist in the filesystem. However, a file
  // pattern is specified that excludes the origin directory. Nothing should be enumerated.
  ...
  // Enumerates the parent directory of a single filesystem rule's origin directory such that the
  // rule's origin directory does not exist but the target directory does exist in the filesystem.
  // That origin directory should be the only item enumerated. It is irrelevant that it does not
  // exist for real in the filesystem.
  ...
  // Enumerates the parent directory of a single filesystem rule's origin directory such that the
  // rule's target directory does not exist but the origin directory does exist in the filesystem.
  ...
  // Enumerates the parent directory of four filesystem rule's origin directories such that two of
  // them have target directories that exist on the real filesystem. Only the two origin
  // directories that belong to filesystem rules with target directories that exist should be
  // enumerated.
  ...
  // Creates two directory enumeration queues and verifies that they are correctly merged, with
  // output properly being provided in sorted order.
  ...
  // Creates two directory enumeration queues and verifies that they are correctly merged, with
  // output properly being provided in sorted order. In this case one queue has entities that all
  // come before entities in the other and hence the queues are drained one at a time.
  ...
  // Creates two directory enumeration queues and verifies that they are correctly merged, with
  // output properly being provided in sorted order. The scan is restarted after getting part-way
  // through it. After the restart all the files should be enumerated.
  ...
  // Creates two directory enumeration queues and verifies that they are correctly merged, with
  // output properly being provided in sorted order. The scan is restarted after getting part-way
  // through it, and the file pattern is changed on restart. Both file patterns should be honored.
  // This test uses a standard directory enumeration queue, rather than a mock, because the former
  // supports query file patterns and this test is intended to ensure that query file patterns are
  // correctly routed to underlying queue objects.
  TEST_CASE(MergedFileInformationQueue_SimpleMergeTwo_DifferentFilePatternOnRestart)
  {
    constexpr FILE_INFORMATION_CLASS kFileInformationClass =
        SFileNamesInformation::kFileInformationClass;
    FileInformationStructLayout layout =
        *FileInformationStructLayout::LayoutForFileInformationClass(kFileInformationClass);
    ...
    auto firstQueue = std::make_unique<EnumerationQueue>(
        InstructionToIncludeAllFiles(),
        kFirstDirectoryName,
  ...
  // progress if at least one underlying queue reports the same. None of the underlying queues
  // report error conditions. They either report "enumeration in progress" or "enumeration done."
  ...
  // Verifies that a merged file information queue correctly reports an enumeration error if any
  // underlying queue reports the same, regardless of what the other underlying queues report.
