...
namespace Pathwinder
{
  ...
  static TemporaryString AccessMaskToString(ACCESS_MASK accessMask)
  {
    ...
    else
    {
      ...
      accessMask &=
          (~(GENERIC_ALL | GENERIC_READ | GENERIC_WRITE | GENERIC_EXECUTE | DELETE |
             FILE_READ_DATA | FILE_READ_ATTRIBUTES | FILE_READ_EA | READ_CONTROL | FILE_WRITE_DATA |
...
#define FILE_CONTAINS_EXTENDED_CREATE_INFORMATION 0x10000000
    ...
    else
    {
      ...
      createOrOpenOptions &=
          (~(FILE_DIRECTORY_FILE | FILE_WRITE_THROUGH | FILE_SEQUENTIAL_ONLY |
             ...
             FILE_SYNCHRONOUS_IO_NONALERT | FILE_NON_DIRECTORY_FILE | FILE_CREATE_TREE_CONNECTION |
             FILE_COMPLETE_IF_OPLOCKED | FILE_NO_EA_KNOWLEDGE | FILE_OPEN_REMOTE_INSTANCE |
             FILE_RANDOM_ACCESS | FILE_DELETE_ON_CLOSE | FILE_OPEN_BY_FILE_ID |
             FILE_OPEN_FOR_BACKUP_INTENT | FILE_NO_COMPRESSION | FILE_OPEN_REQUIRING_OPLOCK |
             FILE_DISALLOW_EXCLUSIVE | FILE_SESSION_AWARE | FILE_RESERVE_OPFILTER |
             FILE_OPEN_REPARSE_POINT | FILE_OPEN_NO_RECALL | FILE_OPEN_FOR_FREE_SPACE_QUERY |
  ...
  static TemporaryString ShareAccessToString(ULONG shareAccess)
  {
    ...
    else
    {
      ...
      if (0 != (shareAccess & FILE_SHARE_WRITE)) outputString << L"FILE_SHARE_WRITE" << kSeparator;
      ...
      CreateDisposition createDisposition)
  {
    ...
    std::optional<OpenHandleStore::SHandleDataView> maybeRootDirectoryHandleData =
        ((nullptr == rootDirectory) ? std::nullopt
                                    : OpenHandleStore::Singleton().GetDataForHandle(rootDirectory));
  ...
  template <typename FileObjectType> TFileOperationsList<FileObjectType> SelectFileOperationsToTry(
      const wchar_t* functionName,
      unsigned int functionRequestIdentifier,
      const FileOperationInstruction& instruction,
  ...
  /// Common internal implementation of hook functions that create or open files, resulting in the
  /// creation of a new file handle. Parameters correspond to the `NtCreateFile` system call, with
  /// the exception of `functionName` which is just the hook function name for logging purposes.
  /// @return Result to be returned to the application on system call completion, or nothing at
  /// all if the request should be forwarded to the application.
      ...
      ULONG eaLength)
  {
    ...
    NTSTATUS preOperationResult =
        ExecuteExtraPreOperations(functionName, requestIdentifier, operationContext.instruction);
          ...
          else
          {
            ...
            if (true == Message::WillOutputMessageOfSeverity(Message::ESeverity::SuperDebug))
              Message::OutputFormatted(
                  ...
                  requestIdentifier,
                  systemCallResult,
                  CreateDispositionToString(ntParamCreateDispositionToTry).AsCString(),
  ...
  /// Parameters correspond to the `NtQueryDirectoryFileEx` system call, with the exception of
  /// `functionName` which is just the hook function name for logging purposes.
  /// @return Result to be returned to the application on system call completion, or nothing at
  /// all if the request should be forwarded to the application.
  static std::optional<NTSTATUS> HookFunctionCommonImplementationQueryDirectoryForEnumeration(
      const wchar_t* functionName,
      HANDLE fileHandle,
      HANDLE event,
      ...
      PUNICODE_STRING fileName)
  {
    ...
    std::optional<OpenHandleStore::SHandleDataView> maybeHandleData =
        OpenHandleStore::Singleton().GetDataForHandle(fileHandle);
...
NTSTATUS Pathwinder::Hooks::DynamicHook_NtClose::Hook(HANDLE Handle)
{
  ...
  std::optional<OpenHandleStore::SHandleDataView> maybeClosedHandleData =
      OpenHandleStore::Singleton().GetDataForHandle(Handle);
  if (false == maybeClosedHandleData.has_value()) return Original(Handle);

  const unsigned int requestIdentifier = GetRequestIdentifier();

  OpenHandleStore::SHandleData closedHandleData;
  NTSTATUS closeHandleResult =
      OpenHandleStore::Singleton().RemoveAndCloseHandle(Handle, &closedHandleData);
    ...
    ULONG EaLength)
{
  ...
  auto maybeHookFunctionResult = HookFunctionCommonImplementationCreateOrOpenFile(
      GetFunctionName(),
      FileHandle,
      DesiredAccess,
      ObjectAttributes,
    ...
    BOOLEAN RestartScan)
{
  ...
  auto maybeHookFunctionResult = HookFunctionCommonImplementationQueryDirectoryForEnumeration(
      GetFunctionName(),
      FileHandle,
      Event,
      ApcRoutine,
    ...
    FILE_INFORMATION_CLASS FileInformationClass)
{
  ...
  // Paths in the open handle store are expected to have a Windows name prefix and begin with a
  // drive letter. According to the documentation for `NtQueryInformationFile` absolute paths
  // begin with a backslash and skip over the drive letter component. So if the actual path is
    ...
    FILE_INFORMATION_CLASS FileInformationClass)
{
  ...
  NTSTATUS preOperationResult =
      ExecuteExtraPreOperations(GetFunctionName(), requestIdentifier, operationContext.instruction);
  if (!(NT_SUCCESS(preOperationResult))) return preOperationResult;
