...
namespace Pathwinder
{
    ...
    template <typename FileObjectType> struct SFileOperationToTry
    {
        ...
        /// Default constructor.
        constexpr SFileOperationToTry(void) : fileObject(nullptr), ntParamCreateDisposition()
        {
            // Nothing to do here.
        }
...
#define FILE_CONTAINS_EXTENDED_CREATE_INFORMATION 0x10000000
    ...
    template <typename FileObjectType> std::array<ValueOrError<SFileOperationToTry<FileObjectType>, NTSTATUS>, 4> SelectFileOperationsToTry(const wchar_t* functionName, unsigned int functionRequestIdentifier, const FileOperationInstruction& instruction, ULONG ntParamCreateDisposition, FileObjectType* unredirectedFileObject, FileObjectType* redirectedFileObject)
    {
        switch (instruction.GetFilenamesToTry())
        {
        case FileOperationInstruction::ETryFiles::UnredirectedOnly:
            return {
                SFileOperationToTry<FileObjectType>(unredirectedFileObject, ntParamCreateDisposition),
            };

        case FileOperationInstruction::ETryFiles::UnredirectedFirst:
            return {
                SFileOperationToTry<FileObjectType>(unredirectedFileObject, ntParamCreateDisposition),
                SFileOperationToTry<FileObjectType>(redirectedFileObject, ntParamCreateDisposition)
            };
