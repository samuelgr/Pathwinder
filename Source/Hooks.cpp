        ...
        else
        {
            ...
            accessMask &=
                (~(GENERIC_ALL | GENERIC_READ | GENERIC_WRITE | GENERIC_EXECUTE | DELETE |
                   ...
                   WRITE_DAC | WRITE_OWNER | SYNCHRONIZE | FILE_EXECUTE | FILE_LIST_DIRECTORY |
                   FILE_TRAVERSE));
            if (0 != accessMask)
...
#define FILE_CONTAINS_EXTENDED_CREATE_INFORMATION 0x10000000
    ...
    )
    {
        ...
        if (true == maybeRootDirectoryHandleData.has_value())
        {
            ...
            std::wstring_view rootDirectoryHandlePath =
                maybeRootDirectoryHandleData->associatedPath;
    ...
    TFileOperationsList<FileObjectType> SelectFileOperationsToTry(
        const wchar_t* functionName,
        unsigned int functionRequestIdentifier,
        const FileOperationInstruction& instruction,
    ...
    )
    {
        ...
        switch (instruction.GetFilenameHandleAssociation())
        {
            case FileOperationInstruction::EAssociateNameWithHandle::None:
            {
                ...
                if (true ==
                    OpenHandleStore::Singleton().RemoveHandle(handleToUpdate, &erasedHandleData))
                    Message::OutputFormatted(
                        Message::ESeverity::Debug,
                    ...
                    else
                    {
                        ...
                        switch (createDispositionToTry.Value().condition)
                        {
                            ...
                            case SCreateDispositionToTry::ECondition::FileMustExist:
                                shouldTryThisFile =
                                    FilesystemOperations::Exists(fileToTryAbsolutePath);
                                break;
...
)
{
    ...
    std::wstring_view systemReturnedFileName =
        GetFileInformationStructFilename(*fileNameInformation);
