...
namespace Pathwinder
{
    namespace Configuration
    {
        ...
        struct MemoryBufferHandle
        {
            ...
            inline MemoryBufferHandle(std::wstring_view configBuffer)
        ...
        )
        {
            ...
            for (; numCharsWritten < std::min(nextLineLength, (lineBufferCount - 1));
                 ++numCharsWritten)
        ...
        std::optional<std::vector<ValueType>> Name::ExtractValues(void)
        {
            if (false == TypeIs<ValueType>()) return std::nullopt;
                ...
                for (const auto& nameRecord : sectionRecord.second.Names())
                {
                    ...
                    for (const auto& valueRecord : nameRecord.second.Values())
                    {
                        switch (valueRecord.GetType())
                        {
                            case EValueType::Integer:
                            case EValueType::IntegerMultiValue:
                                configurationFileString << name << L" = "
                                                        << valueRecord.GetIntegerValue() << L'\n';
                                break;

                            case EValueType::Boolean:
                            case EValueType::BooleanMultiValue:
                                configurationFileString << name << L" = "
                                                        << valueRecord.GetBooleanValue() << L'\n';
                                break;

                            case EValueType::String:
                            case EValueType::StringMultiValue:
                                configurationFileString << name << L" = "
                                                        << valueRecord.GetStringValue() << L'\n';
                                break;
                        }
                    }
                }
