...
#define TEST_FAILED_BECAUSE(reasonf, ...)   do {::PathwinderTest::PrintFormatted(L"%s(%d): Test failed: " reasonf, __FILEW__, __LINE__, ##__VA_ARGS__); TEST_FAILED;} while (0)

/// Exit from a test case and indicate a failing result if the expression is false.
#define TEST_ASSERT(expr)                   do {if (!(expr)) {::PathwinderTest::PrintFormatted(L"%s(%d): Assertion failed: %s", __FILEW__, __LINE__, L#expr); TEST_FAILED;}} while (0)
