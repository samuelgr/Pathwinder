 ...
 *   instance that controls their behavior.
 **************************************************************************************************/
...
#define MOCK_FREE_FUNCTION_MULTICONTEXT_CLASS(classname, numcontexts)                              \
  namespace _MockFreeFunctionContextInternal                                                       \
  {                                                                                                \
    inline constexpr wchar_t kMockFreeFunctionContext__##classname[] = L#classname;                \
  }                                                                                                \
  template <typename MockObjectType> using MockFreeFunctionContextInternalAlias =                  \
      MockFreeFunctionContext<                                                                     \
          MockObjectType,                                                                          \
          _MockFreeFunctionContextInternal::kMockFreeFunctionContext__##classname,                 \
          numcontexts>;                                                                            \
  class classname : public MockFreeFunctionContextInternalAlias<classname>

/// Recommended way of declaring a class that implements a mock context for free functions when only
/// one context is required. Requires a class name.
#define MOCK_FREE_FUNCTION_CONTEXT_CLASS(classname)                                                \
  MOCK_FREE_FUNCTION_MULTICONTEXT_CLASS(classname, 1)

/// Recommended way of implementing a free function whose invocation is fowarded to a mock object
/// controlling the behavior context. Requires a class name, method name, context index, and names
/// of all the parameters to be forwarded.
...
/// name, and names of all the parameters to be forwarded.
#define MOCK_FREE_FUNCTION_BODY(classname, methodname, ...)                                        \
  MOCK_FREE_FUNCTION_MULTICONTEXT_BODY(classname, methodname, 0, __VA_ARGS__)

namespace PathwinderTest
{
  /// Base class for objects that control the behavior of mock versions of free functions.
  /// Instances of subclasses are intended to be created locally within each test case. Subclasses
  /// should be declared using the macro interface provided rather than directly. Instance methods
  /// on subclasses should mirror the free functions they are mocking, and actual free functions
  /// simply need to retrieve an instance and then call the associated instance method.
  /// @tparam MockObjectType Object type that controls the behavior of associated free functions.
  /// Must be a subclass.
  /// @tparam kMockObjectTypeName String representation of the name of the mock object type.
  /// @tparam kNumContexts Number of contexts to create. This can be used to mock free functions
  /// that accept some sort of identifier as a parameter. Defaults to 1.
  template <typename MockObjectType, const wchar_t* kMockObjectTypeName, size_t kNumContexts = 1>
  class MockFreeFunctionContext
  {
  public:
    ...
    MockFreeFunctionContext(size_t index = 0) : contextIndex(index)
    {
      ...
      if (nullptr != contexts[index])
      {
        if constexpr (kNumContexts > 1)
          ...
          TEST_FAILED_BECAUSE(
              L"Multiple instances of mock free function context object %s.", kMockObjectTypeName);
      }
    ...
    /// Retrieves the object currently controlling the context for the specified index, which
    /// defaults to 0. If the instance is missing then the associated test case will fail.
    ...
    static MockObjectType& GetContext(size_t index = 0)
    {
      ...
      if (nullptr == contexts[index])
      {
        if constexpr (kNumContexts > 1)
          ...
          TEST_FAILED_BECAUSE(
              L"Missing instance of mock free function context object %s.", kMockObjectTypeName);
      }
