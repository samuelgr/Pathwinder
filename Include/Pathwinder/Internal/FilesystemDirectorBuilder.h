...
{
  /// Encapsulates all functionality for managing a partially-built filesystem director object,
  /// ensuring consistency between individual filesystem rules, and building a complete filesystem
  /// director object once all rules have been submitted.
  ...
  public:
    ...
    /// Extracts filesystem rules from the specified configuration data object, one rule per
    /// section identified by prefix. If any errors are encountered then log messages are
    /// generated.
    ...
    /// must not end in a backslash. Intended for internal use but exposed for testing.
    ...
    /// Checks if the specified candidate file pattern string is valid for use as a file
    /// pattern. It must not be empty and must not contain any disallowed characters. Intended
    /// for internal use but exposed for testing.
    ...
    /// (1) Rule name must be unique. It cannot match another existing rule in the registry
    /// object. (2) Origin and target directories are not filesystem root directories (i.e. they
    /// both have parent directories). (3) Origin directory must not already be an origin or
    /// target directory for another rule. (4) Target directory must not already be an origin
    /// directory for another rule.
    /// @param [in] ruleName Name of the new rule. Must be unique among rules.
    /// @param [in] originDirectory Origin directory for the new rule. May be relative and
    /// contain references to be resolved.
    /// @param [in] targetDirectory Target directory for the new rule. May be relative and
    /// contain references to be resolved.
    /// @param [in] filePatterns File patterns to narrow the scope of the new rule. This
    /// parameter is optional. Default behavior is to match all files in the origin and target
    /// directories.
    /// @param [in] redirectMode Redirection mode enumerator for the new rule. Determines how
    ...
    /// when adding a rule by supplying its components manually. Internally this method extracts
    /// the rule components and adds the rule using #AddRule.
    ...
    /// between rules, such as relationships between directories, cannot be checked until all
    /// rules have been added. Once a new filesystem director object is built this builder
    /// object is invalidated and should not be used further. Some additional constraints are
    /// imposed on each filesystem rule at build time: (1) Origin directory either exists as a
    /// real directory or does not exist at all (i.e. it does not exist as a file or some other
    /// non-directory entity type). (2) Immediate parent of the origin directory either exists
    /// as a directory or serves as the origin directory for another rule. (3) Target directory
    /// hierarchies are all unrelated. In other words, no target directory has any rule's origin
