...
namespace Pathwinder
{
  ...
  public:
    ...
    /// Prefix for naming auto-generated rules.
    static constexpr std::wstring_view kAutoGeneratedRuleNamePrefix = L"AutoGenerated";
  ...
  private:
    ...
    /// Holds the information needed to create an auto-generated rule.
    struct SAutoGeneratedRuleSpec
    {
      /// Name to assign to the auto-generated rule.
      std::wstring ruleName;

      /// Origin directory for the auto-generated rule.
      std::wstring originDirectory;

      /// Target directory for the auto-generated rule.
      std::wstring targetDirectory;

      inline bool operator<(const SAutoGeneratedRuleSpec& other) const
      {
        return (Infra::Strings::CompareCaseInsensitive<wchar_t>(ruleName, other.ruleName) < 0);
      }
    };

    /// Represents a constraint violation. If it holds an auto-generated rule specification, then
    /// the constraint violation can be fixed by adding a new rule using the supplied
    /// specifications. Otherwise, if it holds a string, then the constraint violation cannot be
    /// satisfied by adding a new rule, and the string describes the reason for the constraint
    /// violation.
    using TConstraintViolation = std::variant<SAutoGeneratedRuleSpec, Infra::TemporaryString>;

    /// Verifies that all of the rules in this object meet constraints that are required for
    /// building a filesystem director object. Any constraints that involve the relationship between
    /// a rule and the real filesystem are checked as part of this process. Stops at the first
    /// constraint violation encountered.
    /// @return A constraint violation object describing the issue, if there is a constraint
    /// violation, or nothing at all if all constraints are satisfied. If an auto-generated rule
    /// specification is returned, then the rule can be added and this function called again.
    std::optional<TConstraintViolation> VerifyPreBuildConstraints(void) const;
