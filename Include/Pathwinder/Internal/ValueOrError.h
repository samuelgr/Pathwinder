...
{
  /// Template for variants that can hold either values or errors. Useful as a return value from
  /// functions that can either produce a result or indicate an error.
  ...
  public:
    ...
    /// If the value type is constructible from a null pointer,  then a null pointer argument
    /// favors construction of a value rather than an error. Useful when both value and error
    /// types can be constructed using a null pointer argument.
    ...
    /// Retrieves a copy of the value held by this object, if this object holds a value, or a
    /// copy of the specified default value otherwise.
    /// @tparam DefaultValueType Type to use for the default value, which must be convertible to
    /// the value type held by this object.
    ...
    /// Moves and returns the value held by this object, if this object holds a value, or a copy
